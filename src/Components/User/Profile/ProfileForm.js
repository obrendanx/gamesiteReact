...
const ProfileForm = ({ onSubmit, initialValues }) => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      setIsLoading(true);
      const response = await axios.get(`https://gamesite-backend.onrender.com/app/api/user/${user.username}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/app/api/userupdate/${details.username}`, updatedFields);
