...
function AnimeCard({anime}) {
    ...
    const fetchUserFavourites = async () => {
      try {
        const response = await axios.get(
          `https://gamesite-backend.onrender.com/app/userfavourites/${user.username}`
        );
    ...
    try {
      ...
      if (isAlreadyFavorite) {
        ...
        const response = await axios.post(
          'https://gamesite-backend.onrender.com/app/newanime',
          newAnimeItem
        );
  ...
  const removeAnimeItem = async (itemId) => {
    try {
      await axios.delete(`https://gamesite-backend.onrender.com/app/removeanime/${user.username}/${itemId}`);
